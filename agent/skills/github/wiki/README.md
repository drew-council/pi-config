...
The wiki is a git repository, `sheerhealth/sheer.wiki.git`, of about 90 flat markdown pages. There is no API for page content, so `wiki.sh` keeps a clone under `~/.cache/sheer-wiki` and reads from it. A page named `Git Worktrees` is the file `Git-Worktrees.md` and the URL `https://github.com/sheerhealth/sheer/wiki/Git-Worktrees`.

## Commands

```sh
wiki.sh list                          # page titles
wiki.sh search "bazel cache"          # pages with matching lines
wiki.sh show "production release process"   # prints the markdown; name is case-insensitive
wiki.sh path "git worktrees"          # local file path
wiki.sh sync                          # force a pull; commands sync on their own once a day
```
