...
Discussions hold anything that needs a conversation before it becomes work: design specs, technical decisions, proposals, questions. `gh` has no discussion command, so `sheer-gh discussion` wraps the GraphQL API.
