...
- `displayMode` has been removed from the tool schema — display mode is user-configured only via `PI_ASK_USER_DISPLAY_MODE` (default `"inline"`, rendered in-place above the input with scrollback visible). Do not attempt to pass a display mode.
