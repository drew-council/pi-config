...
import { type QuestionOption, renderSingleSelectRows } from "./single-select-layout";

const ASK_USER_VERSION = "0.14.0";
...
class AskComponent extends Container {
  ...
  private buildPromptLines(width: number, fullContextLines: string[]): string[] {
    ...
    return [...questionLines, ...(contextLines.length > 0 ? [...contextSeparator, ...contextLines] : [])];
  }
  ...
  private shouldCollapseContextForOverlay(
    ...
    const promptCapacity = Math.max(0, contentRows - separatorRows - this.getMinimumModeRows());
  ...
  private buildEditorModeHeaderLines(width: number): string[] {
    ...
    return [...new Text(this.theme.fg("accent", this.theme.bold("Custom response")), 1, 0).render(width), ""];
  }
  ...
  private limitEditorLines(lines: string[], budget: number, width: number): string[] {
    ...
    const start =
      cursorLineIndex >= 0 ? Math.max(0, Math.min(cursorLineIndex - contentBudget + 1, maxStart)) : maxStart;
...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      ...
      singleSelectLayout: Type.Optional(
        StringEnum(["auto", "list"] as const, {
          description:
            "Single-select layout. 'auto' uses a details pane on wide terminals; 'list' always keeps descriptions below options. Default: PI_ASK_USER_SINGLE_SELECT_LAYOUT if set, otherwise 'auto'.",
        }),
      ),
    ...
    async execute(_toolCallId, params, signal, onUpdate, ctx) {
      ...
      const allowComment =
        requestedAllowComment ?? parseBooleanPreference(process.env.PI_ASK_USER_ALLOW_COMMENT) ?? false;
