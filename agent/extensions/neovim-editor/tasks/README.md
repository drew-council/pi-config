...
factor-out. Tasks 03 and 04 can land either before or right after the move.
...
| 03 | [Reduce prompt history to stock Pi semantics](03-stock-history.md) | this repo | no |
...
The persistent cross-session history currently in `history.ts` moves to a
standalone local extension. That work is described separately in
`agent/extensions/global-history-task.md` and is not part of this sequence.
